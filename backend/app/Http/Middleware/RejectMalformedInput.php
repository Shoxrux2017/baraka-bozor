 ...
 *   with an error that would surface as `500`, the database driver cuts the
 *   second at the NUL and would store half a value, and the framework's
 *   trimming turns invalid UTF-8 into `null` before validation ever sees it.
 *
 * It runs globally, right after the request id and ahead of the trimming.
 */
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if ($request->isJson()) {
            ...
            if (trim($body) !== '' && ! json_validate($body)) {
                throw ApiException::badRequest('malformed_request');
            }
        }

        if (! self::isClean($request->query->all())
            || ! self::isClean($request->request->all())
            || ! self::isClean($request->isJson() ? $request->json()->all() : [])
