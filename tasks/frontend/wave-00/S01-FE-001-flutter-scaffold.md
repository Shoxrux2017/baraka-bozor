...
| R-11 | P3 | The wave index readiness row gave `N/A` for `Auth/scope/security`, although this task ships the bearer-token store that `07` §8 governs; and the new `C:`-full risk row plus change-log row go slightly beyond this contract's stated grant for that file. | **Resolved by the Project Owner, 2026-09-23.** The column is now `Yes`. Three of the four entries are legitimate and stay: the SDK checkbox in the entry gate, the closed SDK risk row, and the wave change-log row. The `C:`-full risk row is removed from this PR — it concerns every track, not this task, so `wave-owner` files it separately; the sentence in the change-log row that announced it went with it, since it would otherwise point at a row that no longer exists. The boundary set for future tasks: **a track edits the rows about its own task in the shared index; anything touching other tracks or the wave as a whole is handed to `wave-owner`.** |
