...
> **SUPERSEDED on 2026-09-21 by `WAVE_00_TASK_INDEX.md`.**
>
> This file is the historical record of the pre-wave decomposition. It is kept unchanged below so the audit trail of what was planned, and in what order, stays readable. It is **not** the execution authority any more.
>
> What changed when it was folded into Wave 0: `S01-BE-001` is `Accepted` (PR #2); `S01-BE-002` is superseded by `W0-BE-011`; the runtime, CI and module-registry tasks were added as `W0-INT-001`, `W0-INT-002` and `W0-BE-010`; `S01-INT-003` was renamed `W0-INT-003`; `S01-BE-006` and the SMS closure criterion in Section 4 and the acceptance map moved to Wave 5 under decision `D-4`, because the vendor contract needs a registered legal entity that does not yet exist.
>
> Do not edit this file. Record changes in `WAVE_00_TASK_INDEX.md`.
