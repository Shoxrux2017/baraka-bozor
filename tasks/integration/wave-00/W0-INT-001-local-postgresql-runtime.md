...
**Resolved 2026-09-21.** Acceptance criterion 6 was provable only by inspection. The Project Owner amended this contract — Decision 5 — to admit one guard test, `backend/tests/Feature/TestDatabaseIsolationTest.php`. The criterion is now enforced by the suite.

### Second review — the guard test, 2026-09-21

Delta review by a second agent with no implementation context. Result: 0 P1, 0 P2, 5 P3. It reproduced both negative cases in container-local copies of the configuration, leaving the tree untouched, and confirmed `phpunit.xml` is byte-identical across `d8c7776`, `e98da75` and the worktree.

| ID | Severity | Finding | Resolution |
|---|---|---|---|
| R-14 | P3 | The guard asserted configuration, never a live connection. `getDriverName()` reads config and `getDatabaseName()` reads a property, and Laravel resolves PDO lazily — so a test named *the suite runs on postgresql* would pass with PostgreSQL stopped. Acceptance criterion 4 says the **active** connection, which was not what was proven. | **Fixed**, and worth more than its P3. Both checks now ask the server: `select version()` and `select current_database()`. Proven by stopping the `db` container and observing both tests fail. |
| R-15 | P3 | "Runs on a dedicated test database" is asserted as "name ends in `_test`"; a developer whose own `.env` also points at a `_test` database would satisfy it and still be wiped. | **Documented.** The docblock now states that this checks the name shape, not difference from the developer's own database. The suffix convention is kept deliberately: a hardcoded name would break in CI and on any rename. |
| R-16 | P3 | `assertNotSame('', $database, ...)` produced no outcome the next assertion did not, and did not cover the `null` it appeared to cover. | **Fixed.** Removed; `assertNotNull` on the query result now covers the real failure it was reaching for. |
| R-17 | P3 | The review section still asked the Project Owner to decide something Decision 5 records as decided. | **Fixed** — this paragraph. |
| R-18 | P3 | Decisions read 1, 2, 3, 5, 4. | **Fixed.** Decision 5 moved after Decision 4. |
