...
| 2026-09-23 | `AUD-028` recorded and independently reviewed: two `P1`, eleven `P2`, eight `P3`, all re-verified before acting. `S01-FE-002` now also stores two sessions, Staff and Customer, for one person on one phone, so the Customer mode decided for Shopper and Courier needs no storage change once clients are installed; the switch control is not Wave 0 (`S-37`). **Wave 0 reopened by `S-32`**, which blocks `S01-FE-005`. No status changed. Merging this with PR #16 needs a hand merge: both edit the Section 4 entry gate, and resolving it either way silently undoes one of two facts — the Flutter SDK being installed, or `S-32` holding `S01-FE-005`. | Project Owner decisions in the `W0-BE-011` follow-up session | Project Owner |
