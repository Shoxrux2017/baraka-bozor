...
| PHPStan fails in CI on `W0-BE-010` with `Child process error (exit code 255)` | `W0-BE-010` and every task behind it | `W0-INT-004` | **Closed 2026-09-22 — cause confirmed.** PHP’s default `memory_limit` of 128M, which the container ran under because `php:8.4-cli` activates no `php.ini`. `W0-INT-004` (PR #11) raises it to 512M, and CI run `35711716471` on head `8919161` passes every step including PHPStan. Measured on this tree after the fix: `phpstan analyse --debug` completes at 190.5 MB — the workload needed half again the memory it was allowed. Worker counts, CPU counts and result caches, all investigated, were never the variable |
...
| 2026-09-22 | `main` merged forward into `W0-BE-010`. The CI failure that held it, `Child process error (exit code 255)`, is closed: the cause was PHP’s default 128M `memory_limit`, fixed by `W0-INT-004` and confirmed by a green run on the merged head. | Re-verification after taking in merged shared infrastructure, root `AGENTS.md` §11 | Project Owner |
