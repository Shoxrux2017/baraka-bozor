...
- Storage for two sessions, Staff and Customer, for one person on one phone (`AUD-028`). The switch control itself is not Wave 0.
...
- [ ] Wave 0 specification decisions all resolved. **Reopened 2026-09-23:** `S-32`, what a role sees on a device that is not its surface, is open and blocks `S01-FE-005`, and would reach `S01-BE-003` and `S01-BE-005` if its answer has the backend enforce it (`AUD-028`). Before that: `S-1` and `S-2` on 2026-09-21 (`AUD-023`); `S-3`, `S-4`, `S-5`, `S-16`, `S-17` and `S-19` on 2026-09-22 (`AUD-024`); `S-28` on 2026-09-22 (`AUD-025`). Resolving `S-1` opened `S-19`; `S-3` opened `S-21` to `S-24`; the independent review of `AUD-024` opened `S-25` to `S-31`, of which `S-28` was the only Wave 0 row. The rest are filed against Waves 1 and 2 and are resolved at those gates — see `docs/SPEC_DECISIONS_BACKLOG.md`. Note that `S-29`, password strength and failed-attempt alerting, is filed in Wave 1 but reaches `09` §8 and §10, which Wave 0 builds; `AUD-025`’s reasoning depends on §10 constraining only length, which is what `S-29` may change.
...
| 6 | `W0-BE-011` | Backend | `wave-owner` | Identity schema: `users` and `customer_otp_challenges`, six roles, `User` model and factory, first-Admin CLI bootstrap, module provider registry | `W0-BE-010` | **Approved** | `backend/wave-00/W0-BE-011-identity-schema.md` |
...
| `W0-BE-011` | Yes | Yes | Yes | Yes | N/A | Yes | Approved |
...
| `S-32` what a role sees on a device that is not its surface | `S01-FE-005`; `S01-BE-003` and `S01-BE-005` if the backend enforces it | decision | **Open** — raised 2026-09-23 and not yet taken up. `frontend/AGENTS.md` §6 and the acceptance matrix already point toward the client refusing a shell on the wrong device. `AUD-028` |
...
| 2026-09-22 | `W0-BE-011` contract written and `Approved`. Four decisions taken with it: `customer_otp_challenges` is created by this schema task, not by `S01-BE-004`, because `backend/database/migrations/**` belongs to `wave-owner` and the `auth-backend` track cannot add a migration without breaking `OWNERSHIP.md`; the first-Admin CLI reads its password by hidden prompt and sets no first-login gate; request-time token behavior — the sliding lifetime and the blocked re-check — stays with `S01-BE-003`; and the task delivers a **module service-provider registry**, extending `D-8` from route files to container bindings so no feature task ever edits `bootstrap/providers.php`. Its short outcome in Section 5 was corrected to match, since "Sanctum tokens" overstated what the task now builds. | Task contract approval, `tasks/README.md` §6 | Project Owner |
| 2026-09-23 | `W0-BE-011` implemented and independently reviewed: fourteen findings, one `P1`, four `P2`, nine `P3`; every `P1` and `P2` fixed. The `P1` was a real credential leak — an unguarded `save()` put the bcrypt hash of the first Admin's password into the command output and twice into `storage/logs/laravel.log`, reproduced and then re-verified fixed by hand. Three findings are left for the Project Owner in the contract's *Reported Separately*: `BR-ROLE-002` role immutability is enforced nowhere, a Staff row with `password = ''` is accepted, and the one-time bootstrap guard is not atomic. All three are schema or policy questions reserved by `AGENTS.md` §3. | Independent review before delivery, `AGENTS.md` §14 | Implementing agent; acceptance remains the Project Owner's |
| 2026-09-23 | The four questions left by `W0-BE-011`'s independent review decided. Role immutability stays an application-layer rule, `09` §44, proven by a test in the Wave 1 Staff administration task (`S-38`); an empty-string Staff password is not a defect; the non-atomic bootstrap guard is an accepted risk; the confirmation prompt is an approved deviation. No code changed. | Project Owner decisions, recorded in the contract | Project Owner |
| 2026-09-23 | `AUD-028` recorded and independently reviewed: two `P1`, eleven `P2`, eight `P3`, all re-verified before acting. `S01-FE-002` now also stores two sessions, Staff and Customer, for one person on one phone, so the Customer mode decided for Shopper and Courier needs no storage change once clients are installed; the switch control is not Wave 0 (`S-37`). **Wave 0 reopened by `S-32`**, which blocks `S01-FE-005`. No status changed. Merged by hand after PR #16 and PR #17: the Section 4 entry gate keeps both facts — the Flutter SDK verified installed, and the specification box unticked by `S-32` — and row 11 keeps `S01-FE-001`'s `In Review` status. | Project Owner decisions in the `W0-BE-011` follow-up session | Project Owner |
