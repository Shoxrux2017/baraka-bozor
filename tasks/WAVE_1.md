...
| A body the JSON decoder cannot read reaches every endpoint as an empty body: a POST then fails its required fields with `422`, and the settings PATCH refuses an empty body, but no endpoint answers the `400 malformed_request` of `docs/09` section 3 | Open; one small middleware before the first panel form (W1-9) |
