...
| S-19 | Open | `W0-BE-011`, `S01-BE-004` | **Customer OTP verify when the only Customer account is blocked.** `09` §7 says verify "resolves or creates" the Customer account. After `AUD-023` the active-Customer index permits a blocked Customer row and an active Customer row on one phone, so when the only Customer row is blocked the contract allows both readings: resolve the blocked row, or create a new active one and thereby void the block. Plain phone uniqueness previously made the question unreachable. Opened by the independent review of `AUD-023`. |
