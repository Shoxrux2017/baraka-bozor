...
| 2026-09-21 | `S-1` resolved: `phone` is unique among active accounts only, via a partial unique index; an account may not be unblocked while another active account holds the same phone (`BR-ROLE-010`). `S-2` resolved: Desktop means an installed Windows application, not a browser; build targets are Android, iOS and Windows with no web target; Android and Windows are required release targets from Wave 0 and iOS becomes required in Wave 5. Recorded as `AUD-023`. |
