...
23. **AUD-023 Phone uniqueness and the Desktop surface** (2026-09-21) — closed `S-1` and `S-2` from `SPEC_DECISIONS_BACKLOG.md`. Project Owner decided both.
    ...
    **This half changes a lifecycle rule and adds a refusal**: unblocking an account is now refusable, as `409 phone_already_active`, which the Project Owner assigned. It also widens account existence — one phone may now carry two active accounts in different families, which no previous text allowed. Both are recorded here because `tasks/README.md` Section 13 has the Project Owner judge evidence validity by exactly this classification.
