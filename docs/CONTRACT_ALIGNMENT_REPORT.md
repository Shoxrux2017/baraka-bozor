...
23. **AUD-023 Phone uniqueness and the Desktop surface** (2026-09-21) — closed `S-1` and `S-2` from `SPEC_DECISIONS_BACKLOG.md`. Project Owner decided both.
    ...
    **`S-1`.** `02` Section 3 required a role change to block the old account and create a new one, while `08` Section 3 made `phone` plainly unique and the person keeps the same number — so the new account could not exist.
    ...
    Changed: `08` Section 3 specifies a partial unique index on `(phone) WHERE status = 'active'`, states the at-most-one-active-account-per-phone invariant, and records that the API still validates the phone so a duplicate returns `422 validation_failed` rather than a constraint violation. `08` Section 31's enforcement checklist follows. `02` Section 3 states the new account reuses the same phone; `02` Section 12 forbids unblocking while another active account holds it and notes that the obvious remedy is unavailable when the conflicting account is the last active Admin. `05` gains `BR-ROLE-010`, scoped to staff login, and `BR-ROLE-002` cites `08` Section 3. `04` Section 12's Admin-creates-Staff flow now validates against active accounts only. `09` Section 8 states that staff login targets the active account.
    ...
    **This half does change a lifecycle rule and adds a refusal**: unblocking an account is now refusable. `09` Section 44 and the code list in Section 59 do **not** yet carry an HTTP status or a stable machine code for it, because assigning those is API semantics reserved to the Project Owner. Until that is decided, no client can branch on the refusal and `D-9` fixtures cannot be written for it. Recorded here as an open follow-up, not as a resolved item.
