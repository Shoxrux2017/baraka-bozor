...
`message` is a **developer-facing English string** and is never shown to a user. The client renders user-facing text from `code`, in the user's language; see `07-architecture.md` Section 27. It follows that **a value a message would need must travel as data, not inside prose**: an error about a minimum order amount carries the amount as a field, so the client can compose the sentence in either language. An endpoint that formats a number into `message` and omits the field ties the client to server-side text permanently, and is a defect.
...
A blocked Customer account is never resolved and never bypassed: verify returns `account_blocked` rather than creating a second active account beside it, so a block cannot be escaped with one SMS. The OTP **request** in Section 6 behaves identically for such a phone — the challenge is created and the SMS is sent — because Section 6 does not disclose account existence, and the refusal belongs at verify. The cost is one SMS; the alternative leaks account status to anyone who can type a phone number.

Verify does **not** create a Cart. The Cart is created lazily on first Cart access, so one place owns the one-active-Cart-per-Customer rule in `08` Section 9 and this endpoint does not change when the Cart arrives. A Customer who never opens the Cart has none.
...
Rate limit: **5 failed attempts per phone per minute** and **20 failed attempts per IP per minute**, both returning `429 rate_limited`. A successful login clears the phone counter. Both limits are required: the per-phone limit alone lets an attacker walk a list of phones, and the per-IP limit alone lets one office lock itself out, since staff share an address.
...
A request whose token is valid but whose account has since been blocked returns `401 account_blocked`. `401` is what a client acts on by discarding the token and returning to the sign-in screen; `403` would invite it to keep a dead token and retry forever.
